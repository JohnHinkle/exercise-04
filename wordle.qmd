...
play_wordle <- function(solution_list, word_length = 5, num_guesses = 6) {
  ...
  print("Enter 'giveu' to quit the game and reveal the solution")
  guess_count <- 0
  ...
  while (num_guesses > guess_count) {
    ...
    while (nchar(guess) != word_length) {
      guess <- readline("Guess must be five letters long! Try another guess:")
    }
    if (identical(guess,"giveu")){
      ##special code to end the game prematurely, or give upgiv
      print(solution)
      break
    }
     guess <- toupper(guess)
     ...
     while (!(guess %in% solution_list)) {
      guess <- readline("This is an invalid guess! Try another guess:")
    }
